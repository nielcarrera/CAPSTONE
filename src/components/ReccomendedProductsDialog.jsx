...
const ProductRecommendationCard = ({ product, onClaim, onDiscard }) => {
  ...
  return (
    <motion.div
      ...
      className={`bg-white rounded-lg shadow-sm overflow-hidden border border-gray-100 hover:shadow-md transition-all duration-300 flex flex-col min-w-[200px]`}
      // Added min-w-[250px] for wider cards
    >
...
const DiscardConfirmationModal = ({ show, onCancel, onConfirm }) => {
  if (!show) return null;
  return (
    <motion.div
      initial={{ opacity: 0 }}
    ...
    >
      <div className="bg-white rounded-lg p-6 max-w-xs w-full shadow-lg">
        <h4 className="text-lg font-semibold mb-3">Discard Product?</h4>
        <p className="text-gray-600 mb-6">
          Are you sure you want to discard this product recommendation?
        </p>
...
const RecommendedProductsDialog = ({
  showDialog,
  onClose,
  recommendedProducts = [],
  skinType,
  ...
  const handleClaim = (productId) => {
    ...
    if (claimedProduct && onClaimProduct) {
  ...
  const handleDiscard = (productId) => {
    setDiscardId(productId);
  };

  const confirmDiscard = () => {
    if (onDiscardProduct && discardId) {
      onDiscardProduct(discardId);
    }
    setProducts((prev) => prev.filter((p) => p.id !== discardId));
    setDiscardId(null);
  };

  const cancelDiscard = () => setDiscardId(null);
    ...
    <>
      ...
      >
        <motion.div
          ...
          animate={{ scale: 1 }}
          exit={{ scale: 0.9 }}
          className="bg-white rounded-lg w-full max-w-6xl mx-4 overflow-y-auto max-h-[90vh] relative"
        >
          <div className="p-6">
            ...
            <div className="mb-4">
              <p className="text-gray-600">
                ...
                {impurity?.value ? ` (${impurity.value}%)` : ""}, and{" "}
            ...
            <div className="mt-6 flex justify-end">
              <button
                onClick={onClose}
                className="px-15 py-3 bg-gray-800 text-white rounded-lg hover:bg-gray-700 transition-colors"
              >
                Finish
              </button>
            </div>

            {/* Success Notification moved inside the dialog, at the bottom */}
...
export default RecommendedProductsDialog;
